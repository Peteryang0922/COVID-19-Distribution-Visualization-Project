...
def search_zip():
    GetLocation = str(input('Do you allow me to get your current location? (Y/N) ')).upper()
    if GetLocation == 'Y':
        import geocoder
        ...
        myloc = [zone['lat'],zone['lng']]
    df_world = catch_distribution()
    df_usa = df_world.loc[(df_world['Country/Region'] == 'US')]
    
    map4 = folium.Map(location = myloc, zoom_start = 10)
    for i,x,y in zip(df_usa['Province/State'],df_usa['Latitude'], df_usa['Longitude']):
        if haversine((x,y), (myloc[0],myloc[1]))/1000 < 50:
            folium.Marker(location=[x,y],popup=i,icon = folium.Icon(color='red', icon='meh-o', prefix='fa')).add_to(map4)
    folium.Marker(myloc, popup='<strong>Your location</strong><br>').add_to(map4)
